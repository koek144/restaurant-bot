...
// Verificar si una mesa está disponible considerando buffer de 2 horas
function isTableAvailable(mesaNumero, fecha, hora) {
  const requestedTime = parseInt(hora.split(':')[0]) * 60 + parseInt(hora.split(':')[1]);

  const conflictingReservations = restaurantData.reservas.filter(r =>
    r.fecha === fecha &&
    r.estado !== 'cancelada' &&
    (r.mesa_numero === mesaNumero || r.mesaIds?.includes(mesaNumero.toString()))
  );

  for (const res of conflictingReservations) {
    const resTime = parseInt(res.hora.split(':')[0]) * 60 + parseInt(res.hora.split(':')[1]);
    const diff = Math.abs(requestedTime - resTime);

    // Si la diferencia es menor a 2 horas (120 minutos), no está disponible
    if (diff < 120) {
      return { available: false, conflictingReserva: res };
    }
  }

  return { available: true, conflictingReserva: null };
}

function getAvailableTables(fecha, hora, ubicacion, personas) {
  return restaurantData.mesas
    .filter(m => m.activa && m.ubicacion === ubicacion && m.capacidad >= personas)
    .filter(m => isTableAvailable(m.numero, fecha, hora).available);
}
